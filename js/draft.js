...
//event listeners
    $('body').on('click', '#launch_new_draft', function(e){
        ...
        config.numTeams = parseInt($('#draft_configuration_select_teams').val());
        config.numRounds = parseInt($('#draft_configuration_select_rounds').val());
        ...
        overall_pick = 1;
        if(flex == 'std'){
            config.allowed_flex_positions = ['RB', 'WR'];
        }
        if(flex == 'te'){
            config.allowed_flex_positions = ['RB','WR','TE'];
        }
        config.teams = [];
        for(i=1;i<=config.numTeams;i++){
            config.teams.push({'team_name':'Team '+i, 'draft_position':i});
        }
...
//functions
    function start_new_draft(config){
        ...
        //console.log(teams_info);
        $('#player_list_filter').load('/php/data.php?resource=player_list_filter');
        get_players();
        filter_player_list('All', 'All');
        //load the team tables based on number of teams and rounds (# of rounds effects bench spots)
        $('#teams_display').load('/php/data.php?resource=teams_display&numTeams='+numTeams+'&numRounds='+numRounds, function(){
            $.each(config.teams, function(){
            $('div[draft_position='+this.draft_position+']').text(this.team_name);
                });
            });
        $('#draft_record').load('/php/data.php?resource=draft_record&numTeams='+numTeams+'&numRounds='+numRounds, function(){
        ...
        $('#quick_draft_configuration').load('/php/data.php?resource=quick_draft_configuration');
        $('#draft_controls').load('/php/data.php?resource=draft_controls');
        $('#draft_status').load('/php/data.php?resource=draft_status&num_rounds='+numRounds+'&num_teams='+numTeams, function(){
